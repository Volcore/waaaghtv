 ...
 *******************************************************************************/
public class InfoRequest implements WC3Message
{
    static Logger   logger = LoggerFactory.getLogger( "InfoRequest" );
    ...
    public InfoRequest( int gameTag, int gameVersion, int gameid )
    {
    ...
    public InfoRequest( IoBuffer in ) throws ParseException
    {
        gameTag = in.getInt( );
    ...
    public byte[] assemble( )
    {
        try {
            ...
            buf.put( WC3Message.UDPINFOREQUEST );
            buf.putShort( length );
            buf.putInt( gameTag );
    ...
    {
        return "("+this.getClass( ).getName( )+": 0x"+HexDump.printHex(gameTag)+" "+gameVersion+" "+gameid+")";
    }
}
